...
export default function QuoteForm({ compact = false, businessType = '' }: QuoteFormProps) {
  ...
  if (compact) {
    return (
      <form action={WORKER_URL} method="POST" className="space-y-3">
        <input type="hidden" name="_subject" value="New Cyber Insurance Quote — CyberCover.co.nz" />
        <input type="hidden" name="_next" value="https://cybercover.co.nz/thank-you/" />
  ...
  return (
    <form action={WORKER_URL} method="POST" className="bg-white rounded-2xl border-2 border-teal-100 p-8 shadow-lg">
